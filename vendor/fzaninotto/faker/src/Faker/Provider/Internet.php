...
class Internet extends \Faker\Provider\Base
{
    ...
    protected static $roll_noFormats = array(
        '{{lastName}}.{{firstName}}',
        '{{firstName}}.{{lastName}}',
        '{{firstName}}##',
        '?{{lastName}}',
    ...
    {
        return preg_replace('/\s/u', '', $this->roll_no() . '@' . static::safeEmailDomain());
    }
    ...
    {
        return preg_replace('/\s/u', '', $this->roll_no() . '@' . static::freeEmailDomain());
    }
    ...
    {
        return preg_replace('/\s/u', '', $this->roll_no() . '@' . $this->domainName());
    }
    ...
    {
        $format = static::randomElement(static::$roll_noFormats);
        $roll_no = static::bothify($this->generator->parse($format));

        return strtolower(static::transliterate($roll_no));
    }
    /**
     * @example 'fY4èHdZv68'
     */
